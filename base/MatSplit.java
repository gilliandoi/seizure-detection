 ...
 */
public class MatSplit {
    private static final Logger log = LoggerFactory.getLogger(MatSplit.class);

    //分割数量指定
    private static int splitAccount = 800;
    ...
    // TXTファイルの拡張子
    protected static String TXT_FLG = ".txt";
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < folders.length; i++) {
            ...
            for (int j = 0; j < files.length; j++) {
                ...
                // matファイルをCSVに変換
                String csvFilePath = outputFolderPath + "/" + files[j].getName().replaceAll(MAT_FLG, "");
                matFileToCSV(files[j].getPath(), csvFilePath);
            }

            log.info("Write：" + outputFolder + " OK!");
        }
...
//            System.out.println(DATA + data);
    ...
    public static void csvOutput(String path, double[][] obj) throws Exception {
        ...
        //時間分割
        //分割個数
        int splitArange = obj[0].length / (splitAccount + 1);

        //ファイル書き込み
        // TODO splitArange→10
        for (int m = 0; m < 10; m++) {
            //出力先指定(ファイル名の連番を０埋め)
            File tempFile = new File(path + "_" + String.format("%04d", m + 1) + TXT_FLG);
            ...
            for (int i = 0; i < obj.length; i++) {
                ...
                //列を書き込み
                for (int j = splitArange * m; j < splitArange * (m + 1); j++) {

                    if (j == splitArange * (m + 1) - 1) {
                        sb.append(ary[j]);
                    } else {
                        sb.append(ary[j] + ",");
                    }
